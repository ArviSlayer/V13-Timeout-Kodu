...
await request.get(`https://discord.com/api/v8/guilds/${message.guild.id}/members/${member.user.id}`, headers, async function (err, data) {
    if (err) throw err;
    if (new Date(JSON.parse(data).communication_disabled_until || Date.now()).getTime() > Date.now()) return message.channel.send("❌・Belirttiğin Kullanıcı Zaten Zaman Aşımına Uğratılmış");
    ...
    // LOG'LAR VE CHAT'E ATILACAK MESAJ BU KISIMDA 
    message.channel.send(`${member.toString()} Adlı Üye, ${message.author} Adlı Yetkili Tarafından, \`${reason}\` Nedeniyle, \`${zaman}\` Boyunca Zaman Aşımına Uğratıldı \`(Ceza ID: #${penal.id})\``);
    return;
});
